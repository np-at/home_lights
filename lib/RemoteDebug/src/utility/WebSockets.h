...
class WebSockets {
    protected:
        ...
        void clientDisconnect(WSclient_t * client, uint16_t code, char * reason = nullptr, size_t reasonLen = 0);
        bool sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload = nullptr, size_t length = 0, bool fin = true, bool headerToPayload = false);

        static  void headerDone(WSclient_t * client);
        ...
        static String base64_encode(uint8_t * data, size_t length);

        static bool readCb(WSclient_t * client, uint8_t *out, size_t n, const WSreadWaitCb& cb);

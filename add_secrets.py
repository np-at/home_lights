...
if not secrets_file:
    ...
    if op_executable is not None:
        try:
            result = subprocess.run(args=[op_executable, 'read', 'op://private/pio_secrets/contents'],
                                    capture_output=True,
                                    text=True)
            result.check_returncode()
            secrets = json.loads(result.stdout).get(pioenv)
        except Exception as ex:
            warnings.warn(f"error while attempting to get secrets from 1Password CLI: {ex}")
...
build_flags = list(map(lambda x: f"-D{x[0]}={format_json_prop(x[1])}", secrets['BUILD_FLAGS'].items()))
print(f"build flags {build_flags}")
env['BUILD_FLAGS'].extend(build_flags)

upload_flags = list(map(lambda x: f"--{x[0]}={format_json_prop(x[1])}", secrets['UPLOAD_FLAGS'].items()))
